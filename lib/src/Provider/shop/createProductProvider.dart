...
class CreateProductProvider with ChangeNotifier {
  ...
  createProduct(String token, int type, BuildContext context) async {
    ...
    Response response = await _utils.post("create-product", 
        body: formData, headerss: headers);

    if (response.statusCode == 200) {
      print("create-product sucsseful");
    ...
    if (_model.code == 200) {
      Future.delayed(Duration(seconds: 1), () {
        ...
        CustomAlert().toast(context: context, title: "تم اضافة منتج بنجاح");
